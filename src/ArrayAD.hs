...
import           Numeric.LinearAlgebra (konst, sumElements, tr, (#>), (<.>))
import           Numeric.LinearAlgebra.Array (Array, None (None))
import           Numeric.LinearAlgebra.Array.Util
  (Coord, asMatrix, asScalar, asVector, fromMatrix, fromVector)
import qualified Numeric.LinearAlgebra.Array.Util as Util
...
ifoldMDelta' :: forall m a r. Monad m
...
foldMDelta' :: forall m a r. Monad m
            => (a -> DualDelta r -> m a)
            -> a
            -> VecDualDelta r
            -> m a
...
buildVector :: forall s r. (Eq r, Num r)
            => Int -> DeltaState r -> Delta r
            -> ST s (Data.Vector.Mutable.MVector s r)
...
valueDual :: (VecDualDelta r -> DeltaMonadValue r a)
          -> Domain r
          -> a
{-# INLINE valueDual #-}
valueDual f ds =
...
valueDualDelta :: (VecDualDelta r -> DeltaMonadValue r (DualDelta r))
               -> Domain r
               -> r
...
gdSmart :: forall r. (Ord r, Fractional r)
        => (VecDualDelta r -> DeltaMonadGradient r (DualDelta r))
        -> Int  -- ^ requested number of iterations
        -> Domain r  -- ^ initial parameters
        -> (Domain' r, r)
...
softMaxActV :: (DeltaMonad r m, Floating r)
            => Data.Vector.Vector (DualDelta r)
...
sumConstantData :: forall m r. (DeltaMonad r m, Num r)
                => Domain r
                -> Int
                -> VecDualDelta r
                -> m (DualDelta r)
...
lossCrossEntropyV
  :: forall m r. (DeltaMonad r m, Floating r)
  => Data.Vector.Vector r
  -> Data.Vector.Vector (DualDelta r)
  -> m (DualDelta r)
lossCrossEntropyV targ res = do
